...
pub enum Command {
    ...
    Decrypt(DecryptArgs),
    /// Internal escape hatch; prefer `export --kind public-key|secret-key|keypair`.
    #[command(hide = true)]
    Keygen(KeygenArgs),
...
#[derive(Debug, Args)]
#[command(about = "Internal escape hatch; use `export --kind public-key|secret-key|keypair` instead")]
